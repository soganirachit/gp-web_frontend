      ...
      >
        ...
        >
          <div className="text-center mb-6">
            <h3 className="text-xl font-semibold text-gray-800 mb-2">Existing Subscription Found</h3>
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="flex justify-between items-center py-2 border-b">
              ...
              <span className="font-semibold capitalize">{subscription?.type?.toLowerCase()}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b">
...
const ProductPage: React.FC = () => {
  ...
  const handleSubscribe = async () => {
    try {
      ...
      const minDays = 7; 
      const pricePerPack = selectedType === 'Daily' 
        ? basePack.sellingPricePerPackDaily 
        : basePack.sellingPricePerPackAlternate;
  ...
  const fetchGarlandProducts = async () => {
    try {
      const allProducts = await productService.getAllProducts();
      const garlandProducts = allProducts
        .filter((product): product is GarlandProduct => 
  ...
  if (error || !basePack) {
    return (
      <div className="flex flex-col items-center justify-center container h-screen">
        <div className="text-red-500 mb-4">{error || 'Base pack not found'}</div>
        <button 
          onClick={() => navigate('/')}
    ...
    <div className="min-h-screen bg-[#FFFBEB]">
      <div className="max-w-[800px] mx-auto relative">
        ...
        <div className="md:flex md:gap-6 md:flex-col">
            ...
            <div className="md:w-1/2 md:pr-4">
              <div className="px-4 py-4">
                <div className="flex justify-between items-start">
                  <h1 className="text-xl font-semibold text-gray-900">{basePack?.name}</h1>
                    ...
                    {basePack?.contents?.map((item, index) => (
                      <span 
                        ...
                        className={`text-sm px-3 py-1 rounded-full ${
                          index % 4 === 0 ? 'bg-[#FFF7E6] text-[#664D03]' :
                          index % 4 === 1 ? 'bg-[#FFF1F2] text-[#881337]' :
                          index % 4 === 2 ? 'bg-[#FFF7ED] text-[#9A3412]' :
          ...
          <div className="mb-8 md:mb-12 ml-4">
            <div className="flex justify-between items-center mb-4 md:mb-6">
              <h2 className="text-xl md:text-2xl font-semibold text-gray-800 ml-4 mt-4" >Garlands</h2>
            </div>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              {showDeliveryDays ? (
                <>
                  ...
                  <div className="mb-6">
                    <h4 className="text-[15px] font-medium mb-4">Select delivery days</h4>
                      ...
                      {weekDays.map((day) => (
                        <button
                          ...
                          className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-medium transition-colors
                            ${!day.enabled ? 'bg-gray-100 text-gray-400' :
                            selectedDays.includes(day.day) ? 'bg-[#015D3A] text-white' :
          ...
          <div className="mb-8 md:mb-12 ml-4">
                    <div className="flex justify-between items-center mb-4 md:mb-6 ">
                      <h2 className="text-xl md:text-2xl font-semibold text-gray-800 mt-4 ml-4">Combo Puja Packs</h2>
                    </div>
                    <div className="flex overflow-x-auto gap-4 no-scrollbar pb-4">
        ...
        <div className='mt-[290px] mb-9 md:mb-9 flex justify-center'>
          <img src={logo} alt='' className='text-[#231F20] opacity-50 md:h-24 md:mb-9' />
        </div>
      </div>
      ...
      <AnimatePresence>
        {showInsufficientBalanceModal && (
          ...
          >
            ...
            >
              <div className="text-center mb-6">
                <h3 className="text-xl font-semibold text-gray-800 mb-2">Insufficient Balance</h3>
                <p className="text-gray-600">
                  You need additional balance to subscribe for {balanceDetails.days} days of {balanceDetails.subscriptionType.toLowerCase()} delivery
                </p>
              </div>
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between items-center py-2 border-b">
                  ...
                  <span className="font-semibold">₹{balanceDetails.currentBalance}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="text-gray-600">Required Amount</span>
                  <span className="font-semibold">₹{balanceDetails.requiredAmount}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b bg-red-50 px-2 rounded">
                  ...
                  <span className="font-semibold text-red-600">₹{balanceDetails.shortageAmount}</span>
                </div>
              </div>

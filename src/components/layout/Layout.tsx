...
const Layout: React.FC = () => {
  ...
  const authRoutes = ['/login', '/otp-verification', '/otp-Verification', '/name-input', '/Name-input', '/allset', '/Allset', '/startup', '/'];
  ...
  const routesWithoutTopPadding = ['/gp-daily'];
  const shouldHideTopPadding = routesWithoutTopPadding.includes(location.pathname);
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEB]">
      ...
      <main className={!isAuthRoute && !shouldHideTopPadding ? "pt-4" : ""}>
        <div className={!isAuthRoute && !hasOwnBottomNav && !shouldHideBottomNav ? "min-h-[calc(100vh-144px)] pb-32" : !isAuthRoute ? "pb-32" : ""}>
          <Outlet />
        </div>
      </main>
